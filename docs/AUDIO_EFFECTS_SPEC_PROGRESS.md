...
- [x] 将高输入下发散的着色多项式替换为有界平滑饱和；母带及最终输出使用立即降增益、50ms 峰值保持、平滑释放的声道联动保护。
- [x] 按 EQ 与低音滤波器的组合频响计算前级余量，避免增强预设长期驱动限幅器；保留频段间的相对增益。
- [x] 母带染色仅驱动湿支路并补偿驱动增益；减去偏置静态输出，并在染色后阻断直流，保证静音输入不自行发声。
...
当前限制器无前视缓冲，立即降增益以避免超峰；峰值保持 50ms，接近已记录峰值（增益差不超过 0.1%）时刷新保持期，随后按预设释放时间恢复。取消输出指数软拐点，安全范围内不再额外压弯波形。参数表保留原攻击时间用于参考核对，当前零延迟限幅不使用它。
它是采样峰值保护，不等于过采样真峰值保证；突发过载的首个上升沿仍可能被改变，稳定播放失真与起始峰值安全分别验证。
EQ/低音前级余量取组合频响峰值的倒数（1024 个对数频点及 DC），只衰减、不补偿负增益；增强预设的整体音量可能降低。混响、响度、声场及母带之后继续保留最终限幅。
...
- [x] C++ ASan/UBSan：20 组预设 × 6 个采样率 × 单/双声道；覆盖热信号、静音、母带 25%/100% 与复合极端增益，检查输出有界/互异、分块一致、重置、饱和曲线和全部 PCM16 数值零参数直通。
- [x] 稳态 THD+N 回归：过载限幅器低于 0.1%，极端 EQ/低音及代表性摇滚、低频、AI 调音曲线低于 0.2%；另检查声道比例、低于限幅阈值透明性和静音后增益恢复。门限不适用于刻意非线性染色的母带预设。
...
g++ -std=c++17 -O1 -g -fsanitize=address,undefined -fno-omit-frame-pointer -Iapp/src/main/cpp app/src/test/cpp/audio-distortion-test.cpp -o /tmp/neko-audio-distortion-test
/tmp/neko-audio-distortion-test
```

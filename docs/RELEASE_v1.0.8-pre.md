...
**预发布限制**：当前没有连接真机，尚未确认设备实际播放与系统音效兼容性。另 14 组基础/声境母带等待原始源码，暂不开放。当前为 PCM16 实时 DSP，不提供真实 AI、杜比多声道解码、过采样真峰值保证或 24-bit 升格。

本次覆盖原 `v1.0.8-pre`，版本号保持不变；已安装同版本的用户需手动下载安装更新。
